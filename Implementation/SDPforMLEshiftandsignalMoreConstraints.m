...
function z = SDPforMLEshiftandsignalMoreConstraints(Y,sigma,DEBUG)
...
for i = 1:N         % signal #
    ...
    for l = 1:L     % rotation 
        ...
        for h = 1:L % hth component of Fourier phase of x
            ...
            C(ind1,ind2_im) = b;
            C(ind2_im,ind1) = b;
            indicesRead(ind1,ind2_re) = 1;
            indicesRead(ind2_re,ind1) = 1;
            indicesRead(ind1,ind2_im) = 1;
            indicesRead(ind2_im,ind1) = 1;
        end
    end
end
...
cvx_begin sdp
    ...
    % constraint on z's: their norm must sum to the power spectrum of z.
    for j = 1:L
...
%         end
	...
	% NEEDS TO BE TESTED
	% constraint: [sum_k u_ik][sum_k u_1k] = 1
% 	for i = 2:N
% 		constraint2 = zeros(N*L+2*L,N*L+2*L);
% 	    for k1 = 1:L
% 		    ind1 = index(1,0,1,k1,0);
% 			for k2 = 1:L
% 			    ind2 = index(1,0,i,k2,0);
% 				constraint2(ind1, ind2) = 1;
% 			end
% 		end
% 		
% 		trace(constraint2*G) == 1
%     end
    
    % constraint 3: toeplitz-like structure?
    
%     for i = 1:N
%         for j = 1:N
%             for l = 1:L
%             for k = 1:L
%                 ind1a = index(1,0,i,mod(k-2,L)+1,0);
%                 ind1b = index(1,0,j,mod(k-2-l,L)+1,0);
%                 ind2a = index(1,0,i,k,0);
%                 ind2b = index(1,0,j,mod(k-l-1,L)+1,0);
%                 G(ind1a,ind1b) == G(ind2a,ind2b)
%             end
%             end
%         end
%     end
